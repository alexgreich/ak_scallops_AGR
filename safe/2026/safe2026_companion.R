...
# table 
read_csv("./data/observer/metadata/ALLScallop_GHL_Dump.csv") %>%
  ...
  left_join(read_csv("./output/safe/2026/meat_weght_removals_time_series_1990_present.csv") %>%
  ...
  mutate(ofl = case_when(year %in% 1996 ~ 1.100,
         ...
         abc = 0.9 * ofl) -> mgmt_qt
# write csv
write_csv(mgmt_qt, "./output/safe/2026/mgmt_quantities_1990_present.csv")
...
ggsave("./figures/safe/2026/fishery_bycatch_comp.png", plot = x + y,
...
bind_rows(ft_1990_2008, obs_2009_present) %>%
  arrange(year) %>%
  write_csv("./output/safe/2026/fishery_stats_1990_present.csv")
  
# mgmt quantities table ----

read_csv("./output/safe/2026/fishery_stats_1990_present.csv") %>%
  mutate(season = paste0(year, "/", substring(year+1, 3, 4))) %>%
  # summarise
  group_by(season, year) %>%
  summarise(ghl = sum(ghl, na.rm = T),
...
read_csv("./output/safe/2026/mgmt_quantities_1990_present.csv") %>%
  ...
  geom_line(data = function(x){filter(x, year %in% 2011:2025)}, aes(x = factor(year), y = ofl*1e6, group = 1))+
...
ggsave("./figures/safe/2026/fishery_catch.png", plot = x,
...
ggsave("./figures/safe/2026/fishery_catch_by_district.png", plot = x,
       height = 3, width = 6, units = "in")
...
read_csv("./output/safe/2026/fishery_stats_1990_present.csv") %>%
  ...
  write_csv("./output/safe/2026/ref_period_avg_by_district.csv")
...
ggsave("./figures/safe/2026/fishery_cpue.png", plot = x,
       height = 3, width = 6, units = "in")
...
bycatch_by_day %>%
  ...
  write_csv("./output/safe/2026/crab_bycatch_2009_present.csv")
