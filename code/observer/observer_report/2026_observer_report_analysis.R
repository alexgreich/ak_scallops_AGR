...
## 2026 observer data summary analysis
## organized by statistic
## Alex Reich- updating the code of Tyler Jackson
...
### custom axis ticks for yrs 
yraxis <- tickr(tibble(yr = 1990:2050), yr, 5)


ak <- geodata::gadm("United States", level = 1, path = "./data/maps")
can <- geodata::gadm("Canada", level = 1, path = "./data/maps")
st_as_sf(ak) %>%
...
# determine average month of the fishery
  ...
  write_csv("./output/observer/2026/fishery_avg_month.csv") #UPDATE EVERY YR. AGR manually created this year file.
...
f_fish_stats(catch, c("KNE"), add_ghl = T, 
             path = "./output/observer/2026/fish_stats_KNE.csv") #AGR manually updated the year file save from 2025 to 2026.
### KSH
...
f_fish_stats(catch, c("O"), add_ghl = T, 
             path = "./output/observer/2026/fish_stats_O.csv")
### Area Q
f_fish_stats(catch, c("Q"), add_ghl = T, 
             path = "./output/observer/2026/fish_stats_Q.csv")
### WKI
...
# trends in round_weight and number of scallops
  ...
  mutate(plot = purrr::map2_chr(data, district, function(data, district){
    ...
    data %>%
      ...
      mutate(scallops = scallops * ifelse(scallops > 0, 1, NA) * 0.4) %>%
      pivot_longer(2:3) %>% 
      mutate(scal_year = substring(season, 1, 4)) %>%
      # plot
      ggplot()+
      geom_point(aes(x = scal_year, y = value, shape = name))+
      geom_line(aes(x = scal_year, y = value, linetype = name, group = name))+
    ...
    ggsave(paste0("./figures/observer/2026/retained_catch_", district, ".png"), #AGR updated the year from 2025 to 2026
           plot = p1, height = 3, width = 7, units = "in")
  })) #AGR flag- this function spits out a few warnings(23 of them). Something is depreaciated but I might not have to deal right now
#AGR - KSW needs the legend moved
...
saveRDS(core, "./output/observer/2026/observer_cpue_std.RDS") #agr updated year file from 2025 to 2026

# diagnostics 

core <- readRDS("./output/observer/2026/observer_cpue_std.RDS") #agr updated year file from 2025 to 2026
...
mapply(function(district, model) {
  ...
  # save plot
  ggsave(paste("./figures/observer/2026/", district, "_observer_cpue_std_diag.png"), plot = qq + rf, height = 5, width = 9, units = "in") #AGR updated year frile from 2025 to 2026
...
ggsave("./figures/observer/2026/observer_cpue_std_yak_effects.png", #AGR updated year from 2025 to 2026
       plot = gridPrint(bed, vessel, dredge_width, ncol = 2),
...
plot(sm(wki_viz, 1))+
  ...
  l_ciLine() +
  labs(x = "Depth (fa)") -> depth
ggsave("./figures/observer/2026/observer_cpue_std_wki_effects.png", #AGR updated year from 2025 to 2026
       plot = gridPrint(depth, ncol = 1),
...
plot(pterm(kne_viz, 2))+
  l_points(col = "grey80") +
  l_ciBar(width = 0.5) + l_fitPoints() +
  labs(x = "Vessel", y = "f(Vessel)") -> vessel

ggsave("./figures/observer/2026/observer_cpue_std_kne_effects.png", #AGR updated year from 2025 to 2026
       plot = gridPrint(depth, bed, month, vessel, ncol = 2),
       height = 5, width = 6, units = "in")
...
plot(pterm(ksh_viz, 3))+
  ...
  l_ciBar(width = 0.5) + l_fitPoints() +
  labs(x = "Month", y = "f(Month)") -> month
plot(pterm(ksh_viz, 4))+
  l_points(col = "grey80") +
  l_ciBar(width = 0.5) + l_fitPoints() +
  labs(x = "Dredge Width", y = "f(Dredge Width)") -> dredge_width
ggsave("./figures/observer/2026/observer_cpue_std_ksh_effects.png", #AGR updated year from 2025 to 2026
       plot = gridPrint(vessel, month,dredge_width, ncol = 2),
       height = 5, width = 6, units = "in")
...
ggsave("./figures/observer/2026/observer_cpue_std_ksw_effects.png", #AGR updated year from 2025 to 2026
       plot = gridPrint(bed, dredge_width, month, ncol = 2),
       height = 5, width = 6, units = "in")
...
bycatch_by_day %>%
  ...
  left_join(cbl %>% 
              ...
              summarise(tanner_cbl = sum(bairdi_bycatch_limit, na.rm = T),
                        snow_cbl = sum(opilio_bycatch_limit, na.rm = T), 
...
bycatch %>%
  ...
  mutate(district = ifelse(district %in% c("UB", "C", "WC"), "M", district)) %>%
  group_by(season, district, set_date, adfg) %>%
  summarise(n_hauls = n(),
            sampled_hauls = sum(sample_hrs > 0),
            dredge_hrs = sum(dredge_hrs, na.rm = T),
            sample_hrs = sum(sample_hrs, na.rm = T),
...
bycatch %>%
  filter(district == "KSH", 
         as.numeric(substring(season, 1, 4)) > 2008) %>%
  group_by(season, district, adfg, set_date) %>%
  summarise(n_hauls = n(),
            sampled_hauls = sum(sample_hrs > 0),
            dredge_hrs = sum(dredge_hrs, na.rm = T),
...
# map
bycatch %>%
    filter(district == "KSH", sample_hrs > 0, clapper_count > 0) %>%
...
# map
bycatch %>%
  filter(district == "KNE", sample_hrs > 0, clapper_count > 0) %>%
  transmute(season, set_lat, set_lon, clapper_cpue = clapper_count/sample_hrs) -> clap_dat
...
bycatch %>%
  ...
  group_by(season, district, set_date) %>%
  summarise(disc_wt = sum(disc_wt, broken_wt, rem_disc_wt, na.rm = T),
...
size_wts %>% 
  ...
  mutate(plot = purrr::map2_chr(data, district, function (data, district){
    ...
    bind_rows(tmp, tmp %>%
      ...
      facet_wrap(~season, ncol = 3, dir = "v", )+
